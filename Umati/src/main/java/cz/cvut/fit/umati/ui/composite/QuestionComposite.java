...
import cz.cvut.fit.umati.ui.utils.ModelViewMapper;
...
public class QuestionComposite extends CustomComponent {
	...
	@Autowired
	private ParameterQueryQuestion parameterQuestion;
	...
	private void showActualQuestionFromQueue() {
		...
		// Set to the panel
		questionPanel.setContent(ModelViewMapper.map(applicationContext, actualEntity));

		// Name label
		if (actualEntity.getClass().isAnnotationPresent(Info.class)) {
			nameLabel.setValue("<h3>" + actualEntity.getClass().getAnnotation(Info.class).description() + "</h3>");
		} else {
			nameLabel.setValue("<h3> No description of model class presents </h3>");
		}
...
//			parameterQuestion.getParameter().setPropertyDataSource(parameterProperty);
//
//			// Set properties for parameters component
...
//			parameterQuestion.getParameterType().setPropertyDataSource(null); // This
//																				// is
//																				// necessary,
//																				// otherwise
//																				// it's
//																				// cleaning
//																				// value!
//			parameterQuestion.getParameterType().setContainerDataSource(xmlDataTypeContainer);
//
//			MethodProperty<XmlDataType> parameterTypeProperty = new MethodProperty<XmlDataType>(actualParameter, "dataType");
//			parameterQuestion.getParameterType().setPropertyDataSource(parameterTypeProperty);
//
//			// Set properties for annotation component
